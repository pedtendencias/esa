RSpec.describe Esa do
  it "has a version number" do
    expect(Esa::VERSION).not_to be nil
  end

	it "can connect to source" do
		expect(Esa.new().connected?).to eq(true)
	end
	...
	context "whilst connected to source" do
		before :all do
			...
			@no_anexo_valido = "RREO-Anexo 04.2"
			@no_anexo_valido_simplificado = "1"
			@no_anexo_valido = "RREO-Anexo 14123"
			@no_anexo_valido_simplificado = "19"
			@codigo_ibge_sao_paulo = 3550308
			@codigo_ibge_amargosa = 2901007
		end

		it "identifies unavailable nr_periodo" do
			data_array = Esa.query_data {an_exercicio: @ano_exercicio,
